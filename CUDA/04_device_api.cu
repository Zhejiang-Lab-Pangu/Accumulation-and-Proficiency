...
void demo0(){
    int deviceCount = 0;
    cudaError_t error_id = cudaGetDeviceCount(&deviceCount);        // Returns the number of compute-capable devices.
    ...
    cudaSetDevice(dev);
    cudaDeviceProp deviceProp;      // the properties of device dev
    cudaGetDeviceProperties(&deviceProp, dev);      // Returns information about the compute-device.
    printf("Device %d:\"%s\"\n", dev, deviceProp.name);
...
//    printf("  Total amount of global memory:                %.2f MBytes (%llu bytes)\n",  (float)deviceProp.totalGlobalMem / pow(1024.0, 3));
    ...
    printf("  Total amount of global memory:                %.2f MBytes ( bytes)\n",  (float)deviceProp.totalGlobalMem / pow(1024.0, 3));
