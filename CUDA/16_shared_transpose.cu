...
__global__ void transformSmem(float *in, float *out, int nx, int ny){
    ...
    bidx = threadIdx.y * blockDim.x + threadIdx.x;      // 此块中线程的坐标的一维表示
    irow = bidx / blockDim.y;       // 转置后的行
    icol = bidx % blockDim.y;       // 转置后的列
