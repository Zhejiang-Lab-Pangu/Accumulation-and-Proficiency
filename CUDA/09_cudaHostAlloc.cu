...
void demo0(int argc, char **argv){
    ...
    // 使用0拷贝内存
    float *a_3_host,*b_3_host,*res_3_d;

    float *res_from_gpu_h_3=(float*)malloc(nByte);

    CHECK(cudaHostAlloc((float**)&a_3_host,nByte,cudaHostAllocMapped));
    CHECK(cudaHostAlloc((float**)&b_3_host,nByte,cudaHostAllocMapped));
    CHECK(cudaMalloc((float**)&res_3_d,nByte));
    initialData(a_3_host,nElem);
    initialData(b_3_host,nElem);

    iStart = cpuSecond();

    sumArraysGPU<<<grid,block>>>(a_3_host,b_3_host,res_3_d);
    CHECK(cudaMemcpy(res_from_gpu_h_3,res_3_d,nByte,cudaMemcpyDeviceToHost));
    iElaps = cpuSecond() - iStart;

    printf("UVA memory elapsed %lf ms \n", iElaps);
    printf("Execution configuration<<<%d,%d>>>\n",grid.x,block.x);

    checkResult(res_h,res_from_gpu_h_3,nElem);
